...
function ProfileHeader({
  ...
  return (
    <section className="mb-5 overflow-hidden border border-slate-800 bg-[#09101d] shadow-[0_14px_40px_rgba(0,0,0,0.22)]">
      ...
      <div className="border-t border-slate-800 bg-[#0a111f] px-4 py-3 sm:px-5">
        <div className="mb-2 flex items-center gap-2">
          <Award className="h-3.5 w-3.5 text-amber-300" />
          <span className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Challenge badges</span>
          <span className="font-mono text-[9px] text-slate-600">{challengeBadges.length}</span>
        </div>

        {challengeBadges.length === 0 ? (
          <div className="border border-dashed border-slate-800 px-3 py-2 font-mono text-[9px] text-slate-700">
            Win a challenge beatmap to earn a permanent badge.
          </div>
        ) : (
          <div className="flex gap-2 overflow-x-auto pb-1">
            {challengeBadges.map((win) => (
              <div
                key={`${win.roundId}-${win.submissionId}`}
                title={`${win.title} — ${win.difficultyName}`}
                className="group relative h-12 w-[92px] shrink-0 overflow-hidden border border-amber-300/55 bg-slate-950 shadow-[0_0_10px_rgba(251,191,36,0.16)] transition hover:border-amber-200 hover:shadow-[0_0_18px_rgba(251,191,36,0.3)]"
              >
                <img
                  src={win.coverUrl}
                  alt={win.title}
                ...
                <div className="absolute inset-x-1 top-1 truncate text-[6px] font-bold uppercase tracking-wider text-white/80 opacity-0 transition group-hover:opacity-100">
                  {win.title}
                </div>
              </div>
            ))}
...
function CareerStats({ profile, rounds }: { profile: ApiPlayerProfile; rounds: ApiPlayerDzppRound[] }) {
  ...
  return (
    <section className="min-w-0">
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden border border-slate-800 bg-slate-800">
        {stats.map(([label, value, accent]) => (
          <div key={label} className={`min-h-[70px] bg-[#0d1526] px-3 py-3 ${accent ? 'bg-amber-300/[0.035]' : ''}`}>
      ...
      <div className="mt-2 border border-slate-800 bg-[#0d1526] px-3 py-3">
        <div className="text-[8px] font-black uppercase tracking-[0.18em] text-slate-600">Profile summary</div>
        <p className="mt-1.5 text-[10px] leading-relaxed text-slate-400">
          {profile.username} has {profile.roundsPlayed} finalized challenge round{profile.roundsPlayed === 1 ? '' : 's'} and {profile.firstPlaces} first-place finish{profile.firstPlaces === 1 ? '' : 'es'}.
        </p>
      </div>
    </section>
  );
            ...
            return (
              <div key={key} className={`border-b border-slate-800/70 last:border-0 ${index === 0 ? 'bg-amber-300/[0.035]' : ''}`}>
                <button
                  ...
                  className="flex w-full items-center gap-2 px-3 py-2.5 text-left transition hover:bg-slate-800/30"
                >
                  <span className="w-5 shrink-0 text-center font-mono text-[8px] text-slate-600">#{index + 1}</span>
                  ...
                  <ChevronDown className={`h-3.5 w-3.5 shrink-0 text-slate-700 transition-transform ${open ? 'rotate-180' : ''}`} />
                </button>
                {open && (
                  <div className="grid grid-cols-2 gap-px border-t border-slate-800/70 bg-slate-800/60 sm:grid-cols-5">
                    <MiniStat label="Score" value={perf.score.toLocaleString()} />
                    <MiniStat label="Accuracy" value={`${perf.accuracy.toFixed(2)}%`} />
                    <MiniStat label="Misses" value={String(perf.misses)} />
                    <MiniStat label="Combo" value={`${perf.maxCombo.toLocaleString()} / ${perf.beatmapMaxCombo.toLocaleString()}`} />
                    <MiniStat label="Round" value={`R${perf.roundNumber}`} />
                  </div>
                )}
              </div>
            );
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[#0b1220] px-2.5 py-2">
      <div className="font-mono text-[7px] uppercase tracking-wider text-slate-600">{label}</div>
      <div className="mt-1 truncate font-mono text-[9px] font-bold text-slate-300">{value}</div>
    </div>
  );
}

function RecentRounds({ rounds }: { rounds: ApiPlayerDzppRound[] }) {
  const [showAll, setShowAll] = useState(false);
  const visible = showAll ? rounds : rounds.slice(0, 4);
  return (
    <section className="min-w-0">
      <SectionHeader
        title="Recent Rounds"
        action={rounds.length > 4 ? (showAll ? 'Collapse' : 'See all') : undefined}
        onAction={() => setShowAll((value) => !value)}
      />
      <div className="overflow-hidden border border-slate-800 bg-[#0d1526]">
        {visible.length === 0 ? (
          <EmptyState message="No challenge history yet." />
        ) : (
          visible.map((round) => (
            <div key={round.roundId} className="flex items-center gap-2 border-b border-slate-800/70 px-3 py-3 last:border-0">
              <div className="w-16 shrink-0">
                <div className="font-mono text-[9px] text-slate-500">{monthLabel(round)}</div>
                <div className="mt-0.5 font-mono text-[8px] text-slate-700">R{round.roundNumber}</div>
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-[10px] font-bold text-white">{round.maps.length} challenge map{round.maps.length !== 1 ? 's' : ''}</div>
                <div className="mt-1"><PlacementBadge placement={round.placement} /></div>
              </div>
              <div className="shrink-0 text-right">
                <div className="font-mono text-[11px] font-black text-amber-300">+{round.finalDzpp}</div>
                <div className="font-mono text-[7px] uppercase tracking-widest text-slate-700">DZPP</div>
              </div>
            </div>
          ))
        )}
        {rounds.length > 4 && (
          <button type="button" onClick={() => setShowAll((value) => !value)} className="w-full border-t border-slate-800 py-2 text-[9px] font-bold text-slate-600 hover:text-white">
            {showAll ? 'Show fewer' : 'See all rounds'}
          </button>
        )}
      </div>
    </section>
          ...
          return (
            <React.Fragment key={round.roundId}>
              ...
              >
                ...
                <span className="min-w-0 truncate text-[9px] font-bold text-white">Round {round.roundNumber} · {round.maps.length} maps</span>
...
function ShopCollection({
  ...
  const equipped = ownedItems.filter(
    (item) =>
      item.profileSlot !== null &&
      equippedIds.get(item.profileSlot) === item.itemId,
  );
...
export default function PlayerProfilePage({ username, onBack }: PlayerProfilePageProps) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-3 py-4 pb-16 text-slate-100 sm:px-5 lg:px-7">
      ...
      <ProfileHeader profile={profile} shopProfile={shopProfile} challengeBadges={displayedChallengeWins} />
      ...
      {!roundsReady || historyFailed ? (
        ...
        <>
          <div className="mb-5 grid gap-4 lg:grid-cols-[1.05fr_1.55fr_1.05fr]">
