...
// than a cast.
import { ApiBeatmapPreview, ApiFavorite, ApiSearchDifficulty, ApiSearchHit, ApiSubmission, } from '../api/client';
...
export function toBeatmap(submission: ApiSubmission): Beatmap {
  return {
    ...
    cs: submission.cs ?? undefined,
    ar: submission.ar ?? undefined,
    od: submission.od ?? undefined,
    hp: submission.hp ?? undefined,
 ...
 */
export function searchHitToBeatmap(
  hit: ApiSearchHit,
  difficulty: ApiSearchDifficulty
): Beatmap {
return {
  id: `search-${difficulty.difficultyId}`,
  ...
  bpm: difficulty.bpm,
  length: formatLength(difficulty.lengthSeconds),
...
return {
  ...
  cs: favorite.cs ?? undefined,
  ar: favorite.ar ?? undefined,
  od: favorite.od ?? undefined,
  hp: favorite.hp ?? undefined,
  isFavorited: true,
};
}
