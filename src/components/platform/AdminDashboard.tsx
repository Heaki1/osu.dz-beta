...
if (!rows.ok) {
  setError(`Could not read countries: ${rows.error}`);
  return;
}

if (!overrides.ok) {
  setError(`Could not read user exceptions: ${overrides.error}`);
  return;
}

setError(null);
setCountries(rows.data);
setExceptions(overrides.data);
  }, []);
...
function BeatmapRulesTab() {
  ...
  useEffect(() => {
    ...
    setMaxSubmissionsPerUser(
      settings.maxSubmissionsPerUser === null ? '' : String(settings.maxSubmissionsPerUser)
    );
  }, [settings]);
  ...
  const handleSave = async () => {
    ...
    let maxSubmissionsPerUserValue: number | null;
    const trimmedMaxSubmissionsPerUser = maxSubmissionsPerUser.trim();
    if (trimmedMaxSubmissionsPerUser === '') {
      maxSubmissionsPerUserValue = null;
    } else {
      const n = Number(trimmedMaxSubmissionsPerUser);
      if (!Number.isInteger(n) || n < 1) {
        setLocalError('Max submissions per user must be a positive whole number, or empty for no limit.');
        return;
      }
      maxSubmissionsPerUserValue = n;
    }
    ...
    await save({
      ...
      maxSubmissionsPerUser: maxSubmissionsPerUserValue,
    });
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <Section
        ...
        description="How many non-rejected submissions each user may make in a single round. Leave empty for no limit. Rejected submissions do not count toward this limit."
      >
        <BoundInput
          label="Limit (empty = no limit)"
          value={maxSubmissionsPerUser}
          placeholder="no limit"
          onChange={setMaxSubmissionsPerUser}
        />
      </Section>
...
function ChallengeTab() {
  ...
  useEffect(() => {
    ...
    setMaxChallengeBeatmaps(
      settings.maxChallengeBeatmaps === null ? '' : String(settings.maxChallengeBeatmaps)
    );
  }, [settings]);
  ...
  const handleSave = async () => {
    ...
    let maxChallengeBeatmapsValue: number | null;
    const trimmed = maxChallengeBeatmaps.trim();
    if (trimmed === '') {
      maxChallengeBeatmapsValue = null;
    } else {
      const n = Number(trimmed);
      if (!Number.isInteger(n) || n < 1) {
        setLocalError('Challenge beatmaps per round must be a positive whole number, or empty for no limit.');
        return;
      }
      maxChallengeBeatmapsValue = n;
    }

    setLocalError(null);
    await save({ allowedMods: mods, allowedChallengeTypes: types, maxChallengeBeatmaps: maxChallengeBeatmapsValue });
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <Section
        title="Challenge Beatmaps per Round"
        description="How many of the highest-voted approved submissions advance to the challenge phase when the winner is approved. All approved submissions still appear in the voting ballot — this only controls how many enter the challenge. Leave empty to include all approved submissions."
      >
        <BoundInput
          label="Limit (empty = all approved)"
          value={maxChallengeBeatmaps}
          placeholder="no limit"
          onChange={setMaxChallengeBeatmaps}
        />
        <p className="text-[11px] text-slate-500 mt-2">
          Example: 8 submissions approved, limit set to 4 — the 4 with the most votes become
          challenge beatmaps. The other 4 remain in the voting history but are not played.
          Ties at the cut-off are broken by submission id (lower id wins).
        </p>
      </Section>
