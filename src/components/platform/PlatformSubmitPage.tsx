...
import {
  ...
  Link, Heart, ChevronRight, X, AlertCircle, LogIn, Trash2,
} from 'lucide-react';
...
function UrlTab({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(false);
    if (result.ok) {
      // Reset the form so the submitted beatmap doesn't linger on screen
      setUrl('');
      setPreview(null);
      setMod(null);
...
function FavoritesTab({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(false);
    if (result.ok) {
      // Reset the picker and reload favorites so the submitted beatmap
      // doesn't remain in the list as if nothing happened.
      reset();
...
/** Compact submission card with status badge, requirements, and red withdraw button. */
function MySubmission({
  ...
  const handleWithdraw = async () => {
    ...
    const err = await onWithdraw(submission.id);
    setWithdrawing(false);
    if (err) setWithdrawError(err);
  };

  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-800 bg-[#0d1526]">
      {/* Beatmap card */}
      <BeatmapCardPlatform beatmap={toBeatmap(submission)} />

      {/* Requirements + meta */}
      <div className="flex flex-col gap-3 px-4 py-3 border-t border-slate-800/60">
        {/* Status badge */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${copy.tone}`}>
            {copy.label}
          </span>
          <p className="text-[11px] text-slate-500 leading-tight">{copy.blurb}</p>
        </div>

        {/* Requirement pills */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-black font-mono bg-slate-800 border border-slate-700 px-2.5 py-1 rounded-lg text-white">
            {submission.modRequirement}
          </span>
          <span className="text-xs font-bold bg-amber-400/10 border border-amber-400/25 px-2.5 py-1 rounded-lg text-amber-400">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="ml-auto inline-flex items-center gap-1 text-[11px] text-slate-600 hover:text-amber-400 transition-colors"
          >
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 w-full px-4 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 hover:border-rose-500/50 text-rose-400 text-xs font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        >
...
const handleLoadFavorites = async () => {
  ...
  <div className="mb-8">
    <p className="text-[10px] uppercase tracking-widest text-slate-600 font-mono mb-3">Your Submissions</p>
