        ...
        {user ? (
          <div ref={profileMenuRef} className="relative">
            <button
              ...
              aria-haspopup="menu"
              className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-800 border border-slate-700 hover:border-slate-600 transition-all"
            >
            ...
            {profileMenuOpen && (
              <div
                role="menu"
                className="absolute top-full right-0 mt-2 w-48 bg-[#0d1526] border border-slate-800 rounded-xl shadow-xl z-50 overflow-hidden"
              >
                <div className="px-4 py-3 border-b border-slate-800">
                  <p className="text-xs font-bold text-white">{user.username}</p>
                  <p className="text-[10px] text-slate-500">
                    {user.country} · {formatRank(user.rank)}
                  </p>
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    window.location.assign(
                      `/player/${encodeURIComponent(user.username)}`
                    );
                  }}
                  className="w-full flex items-center gap-2 px-4 py-2.5 text-xs text-slate-300 hover:text-amber-400 hover:bg-amber-400/5 transition-all text-left"
                >
