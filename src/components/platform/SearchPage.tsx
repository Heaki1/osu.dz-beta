...
import { searchHitToBeatmap } from '../../lib/submission';
...
{results.map((hit) => {
  const difficulties = hit.difficulties.map((difficulty) => ({
 ...searchHitToBeatmap(hit, difficulty),
    isFavorited: favoritedIds.has(difficulty.difficultyId),
  }));

  return (
    <div key={hit.beatmapsetId} className="flex flex-col gap-1.5">
      <BeatmapCardPlatform
  beatmap={difficulties[0]}
  difficulties={difficulties}
  onFavorite={(map) => {
    if (map) onFavorite(map);
  }}
/>
