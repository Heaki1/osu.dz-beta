...
router.post('/round/winner', async (req, res) => {
  ...
  if (submissionId !== undefined) {
    const id = parsePositiveInt(submissionId);
    if (id === null) {
...
router.post('/challenge/scores', async (req, res) => {
  ...
  const osuId = parsePositiveSafeInt(body.osuId);
  if (osuId === null) {
    res.status(400).json({ error: 'osuId must be the player osu! id' });
    return;
  }

  const score = body.score === 0 || body.score === '0'
    ? 0
    : parsePositiveSafeInt(body.score);
  ...
  // POST /api/admin/challenge/scores
...
if (roundId === null) {
  ...
  try {
    const rows = await listRecomputes(roundId);
...
router.post('/dzpp/recompute', async (req, res) => {
  ...
  const id = parsePositiveInt(roundId);
if (id === null) {
...
//
// When approving, the cap from site_settings.max_challenge_beatmaps is checked first.
// Re-approving an already-approved submission does not count against the cap.
...
if (id === null) {
  ...
  try {
    const updated = await review(id, status as ReviewDecision, reviewer.id);
    if (!updated) {
...
router.put('/settings', async (req, res) => {
  ...
  // maxChallengeBeatmaps: a positive integer, or null to remove the cap.
  if (body.maxChallengeBeatmaps !== undefined) {
    if (body.maxChallengeBeatmaps === null || body.maxChallengeBeatmaps === '') {
      patch.maxChallengeBeatmaps = null;
    } else {
      const n = Number(body.maxChallengeBeatmaps);
      if (!Number.isInteger(n) || n < 1 || n > 10_000) {
        res.status(400).json({ error: 'maxChallengeBeatmaps must be a positive integer, or null for no limit' });
        return;
      }
      patch.maxChallengeBeatmaps = n;
    }
  }
  // maxSubmissionsPerUser: a positive integer, or null to remove the cap.
  if (body.maxSubmissionsPerUser !== undefined) {
    if (body.maxSubmissionsPerUser === null || body.maxSubmissionsPerUser === '') {
      patch.maxSubmissionsPerUser = null;
    } else {
      const n = Number(body.maxSubmissionsPerUser);
      if (!Number.isInteger(n) || n < 1 || n > 10_000) {
        res.status(400).json({
          error: 'maxSubmissionsPerUser must be a positive integer, or null for no limit',
        });
        return;
      }
      patch.maxSubmissionsPerUser = n;
    }
  }
...
router.post('/dzp/adjustment', async (req, res) => {
  const body = (req.body ?? {}) as Record<string, unknown>;
  ...
  const amount =
    typeof body.amountDzp === 'number' || typeof body.amountDzp === 'string'
      ? Number(body.amountDzp)
      : NaN;

  if (!Number.isInteger(amount) || amount === 0 || Math.abs(amount) > 1_000_000) {
    res.status(400).json({
      error: 'amountDzp must be a non-zero integer between -1000000 and 1000000',
    });
    return;
  }
  ...
  try {
    ...
    if (!target) {
      res.status(404).json({
        error: 'No such account. A player must have signed in at least once.',
      });
      return;
    }

    const adjustment = await addAdminDzpAdjustment(
      userId,
      amount,
...
router.get('/shop/items', async (_req, res) => {
  try {
    const rows = await listShopItemsAdmin();
        ...
        artwork:
          row.asset_id === null
            ...
            : {
                ...
                isAnimated: row.asset_is_animated ?? false,
              },
...
router.post('/shop/items', async (req, res) => {
  ...
  if (
    ...
    (ownershipType !== 'normal' && ownershipType !== 'stealable') ||
    typeof displayOrder !== 'number' ||
    !Number.isInteger(displayOrder) ||
    typeof initialPriceDzp !== 'number' ||
    !Number.isInteger(initialPriceDzp)
  ) {
    res.status(400).json({ error: 'Invalid Shop draft fields' });
    return;
  }

  try {
    const draft = await createShopItemDraft({
      name,
      description,
    ...
    fail(res, err, 'Shop draft creation');
  }
});

router.get('/shop/items/:itemId/assets', async (req, res) => {
  try {
    const rows = await getShopItemAssetsAdmin(req.params.itemId);
    res.json(rows.map((row) => ({
  id: row.id,
  assetType: row.asset_type,
  url: row.url,
  altText: row.alt_text,
  frameInnerDiameterRatio: frameRatio(row.frame_inner_diameter_ratio),
  isAnimated: row.is_animated,
  isActive: row.is_active,
  createdAt: row.created_at,
})));
  } catch (err) {
    fail(res, err, 'Shop artwork list');
  }
});

router.post('/shop/items/:itemId/assets', async (req, res) => {
  const {
  assetType,
  url,
  altText,
  frameInnerDiameterRatio: rawFrameInnerDiameterRatio,
  isAnimated,
  makeActive,
} = (req.body ?? {}) as Record<string, unknown>;

const frameInnerDiameterRatio =
  rawFrameInnerDiameterRatio === undefined ||
  rawFrameInnerDiameterRatio === null ||
  rawFrameInnerDiameterRatio === ''
    ? null
    : Number(rawFrameInnerDiameterRatio);
...
if (
  ...
  typeof isAnimated !== 'boolean' ||
  typeof makeActive !== 'boolean' ||
  (frameInnerDiameterRatio !== null &&
    (!Number.isFinite(frameInnerDiameterRatio) ||
      frameInnerDiameterRatio <= 0 ||
      frameInnerDiameterRatio > 1))
) {
    res.status(400).json({ error: 'Invalid Shop artwork fields' });
    return;
  }

  try {
    await createShopItemAssetAdmin(req.params.itemId, {
  assetType,
  url,
  altText,
  ...
  makeActive,
    ...
    fail(res, err, 'Shop artwork creation');
  }
});

router.patch('/shop/items/:itemId/assets/:assetId/active', async (req, res) => {
  try {
    await setShopItemAssetActiveAdmin(req.params.itemId, req.params.assetId);
    res.json({ ok: true });
  } catch (err) {
    fail(res, err, 'Shop artwork activation');
  }
});

router.patch('/shop/items/:itemId/lifecycle', async (req, res) => {
  const { itemId } = req.params;
  const lifecycle = (req.body as { lifecycle?: unknown } | undefined)?.lifecycle;
  ...
  if (
    ...
    res.status(400).json({ error: 'Invalid Shop item lifecycle' });
    return;
  }

  try {
    await updateShopItemLifecycleAdmin(itemId, lifecycle);
    res.json({ ok: true });
  } catch (err) {
    fail(res, err, 'Shop item lifecycle update');
  }
});
...
if (
  ...
  (currentPriceDzp !== null &&
    (typeof currentPriceDzp !== 'number' ||
      !Number.isInteger(currentPriceDzp)))
) {
  res.status(400).json({ error: 'Invalid Shop item fields' });
  return;
}

  try {
    await updateShopItemAdmin(itemId, {
      name,
      description,
      displayOrder,
      initialPriceDzp,
      currentPriceDzp: currentPriceDzp as number | null,
    });

    res.json({ ok: true });
  } catch (err) {
    fail(res, err, 'Shop item update');
  }
});
