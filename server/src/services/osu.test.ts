...
describe('orderHits', () => {
  const hit = (stars: number, bpm: number): OsuSearchHit => ({
    beatmapsetId: Math.round(stars * 100),
    title: 'title',
    artist: 'artist',
    mapper: 'mapper',
    mapStatus: 'ranked',
  ...
  it('orders by stars, hardest first', () => {
    const ordered = orderHits(
      [hit(4, 200), hit(7, 150), hit(5.5, 180)],
      'stars'
    );

    expect(ordered.map((h) => h.difficulties[0]?.stars)).toEqual([7, 5.5, 4]);
  });
  ...
  it('orders by bpm, fastest first', () => {
    ...
    expect(ordered.map((h) => h.difficulties[0]?.bpm)).toEqual([200, 180, 150]);
  });
  ...
  it('leaves the array it was given alone', () => {
    ...
    expect(hits.map((h) => h.difficulties[0]?.stars)).toEqual([4, 7]);
  });
});
...
describe('orderHits with the four sorts', () => {
  const hit = (stars: number, bpm: number, title: string): OsuSearchHit => ({
    beatmapsetId: Math.round(stars * 100),
    title,
    artist: 'artist',
    mapper: 'mapper',
  ...
  it('orders by stars descending', () => {
    expect(orderHits(hits, 'stars').map((h) => h.difficulties[0]?.stars))
      .toEqual([7, 5, 4]);
  });

  it('orders by BPM descending', () => {
    expect(orderHits(hits, 'bpm').map((h) => h.difficulties[0]?.bpm))
      .toEqual([200, 180, 150]);
  });
  ...
  it('leaves the osu! order alone for relevance and newest', () => {
    expect(orderHits(hits, 'relevance').map((h) => h.difficulties[0]?.stars))
      .toEqual([4, 7, 5]);

    expect(orderHits(hits, 'newest').map((h) => h.difficulties[0]?.stars))
      .toEqual([4, 7, 5]);
  });

  it('never mutates the array it was given', () => {
    const before = hits.map((h) => h.difficulties[0]?.stars);

    orderHits(hits, 'stars');

    expect(hits.map((h) => h.difficulties[0]?.stars))
      .toEqual(before);
  });
});