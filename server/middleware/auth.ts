 ...
 *
 * A missing, invalid, deleted, or revoked session is treated as signed out
 * rather than rejected. Database failures still return 503 because the request
...
export async function optionalAuth(
  ...
  try {
    ...
    console.error('[auth] optional user lookup failed:', err instanceof Error ? err.message : err);
    res.status(503).json({ error: 'Database unavailable' });
    return;
  }

  if (!user || claims.epoch !== user.session_epoch) {
    next();
    return;
  }
