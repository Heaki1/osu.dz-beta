...
// DZPP qualification point values shown on challenge cards only.
const MOD_COMPLIANCE_DZPP = 10;
const CHALLENGE_REQUIREMENT_DZPP = 15;

// ── Star colour scheme — osu!-inspired difficulty progression ────────────────
function starColorScheme(rating: number): { header: string; fill: string; border: string } {
  ...
  if (rating < 2.5) return {
    header: 'bg-cyan-400 text-slate-950',
    fill: 'bg-cyan-400',
    border: 'border-cyan-400/60',
  };

  if (rating < 3.0) return {
    header: 'bg-green-500 text-slate-950',
    fill: 'bg-green-400',
    border: 'border-green-500/60',
  };

  if (rating < 3.5) return {
    header: 'bg-lime-400 text-slate-950',
    fill: 'bg-lime-400',
...
interface BeatmapCardProps {
  ...
  /** Hide the comments control/panel for read-only collection cards. */
  showCommentsButton?: boolean;
  /** Optional gift action for transferable collection cards. */
  onGift?: () => void;
  /** Collection cards show requirements as collected-map metadata, not DZPP rewards. */
  isCollectionCard?: boolean;
  /** DZPP earned from this challenge beatmap, shown on collected cards. */
  collectionDzpp?: number;
  /** Collection owner earned both the approved-submission and vote completion points. */
  collectionPerfectionEligible?: boolean;
...
export const BeatmapCard: React.FC<BeatmapCardProps> = ({
  ...
  onInspectCard, showCommentsButton = true, onGift, isCollectionCard = false, collectionDzpp,
  collectionPerfectionEligible = false,
}) => {
      ...
      >
        ...
        <div className={`card-face bg-[#0f172a] border rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col text-slate-100 group ${starBorderClass}`}>
          ...
          <div className="relative h-28 w-full bg-slate-900 overflow-hidden flex-shrink-0">
            ...
            {/* Mod badge */}
            <span className={`absolute top-2.5 left-2.5 text-[10px] uppercase font-black tracking-wider px-2 py-0.5 rounded-full border backdrop-blur-md ${ms.bg} ${ms.border} ${ms.text}`}>
              {ms.label}
            </span>
          ...
          <div className="mt-auto pt-3 px-4 pb-4 border-t border-slate-800 flex items-center gap-2 flex-shrink-0">
            ...
            {onGift && (
              <button
                ...
                className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-fuchsia-400/40 bg-fuchsia-500/10 py-2 text-xs font-bold text-fuchsia-300 transition-colors hover:border-fuchsia-400/70 hover:bg-fuchsia-500/20"
                title="Gift this challenge map"
              >
        ...
        <div className={`card-face card-face-back bg-[#0f172a] border rounded-2xl overflow-hidden shadow-lg flex flex-col text-slate-100 ${starBorderClass}`}>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
            ...
            <p className="text-[9px] uppercase tracking-widest text-slate-500 font-bold mb-1">
              {isCollectionCard ? 'Challenge Requirement' : 'Complete challenges · Earn DZPP'}
            </p>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {!isCollectionCard ? (
                        <span className="text-[10px] font-mono font-black text-amber-400 flex-shrink-0">
                          +{CHALLENGE_REQUIREMENT_DZPP} dzpp
                        </span>
                      ) : null}
                    </div>
            ...
            {isCollectionCard && collectionPerfectionEligible && (
              <div className="rounded-xl border border-emerald-500/30 bg-emerald-950/20 p-3">
                <div className="flex items-start gap-2.5">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 border border-emerald-500/40 bg-emerald-900/30 text-emerald-400">
                    <Trophy className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[11px] font-bold text-emerald-300 leading-tight">Perfection</p>
                    </div>
                    {collectionDzpp != null && (
                      <div className="mt-2 flex items-center justify-between gap-2">
                        <span className="text-[10px] text-emerald-200/60">challenge dzpp earned</span>
                        <span className="text-[10px] font-mono font-black text-amber-400">+{collectionDzpp} dzpp</span>
                      </div>
                    )}
                  </div>
                </div>
