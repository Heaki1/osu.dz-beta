...
import type { ApiFavorite, ApiSearchHit, ApiSubmission, ApiSearchDifficulty} from '../api/client';
...
const difficulty = (over: Partial<ApiSearchDifficulty> = {}): ApiSearchDifficulty => ({
  difficultyId: 4907876,
  difficultyName: 'Expert',
  stars: 5.67,
  bpm: 180,
  ...
  it('prefixes the id so it cannot collide with a submission id', () => {
    expect(searchHitToBeatmap(hit(), difficulty()).id).toBe('search-4907876');
  });

  it('formats the raw seconds the way the card expects', () => {
    expect(searchHitToBeatmap(hit(), difficulty()).length).toBe('4:06');
    expect(
      searchHitToBeatmap(hit(), difficulty({ lengthSeconds: 63 })).length
    ).toBe('1:03');
  });

  it('carries the map status through as the card status', () => {
    expect(
      searchHitToBeatmap(hit({ mapStatus: 'loved' }), difficulty()).status
    ).toBe('loved');
  });

  it('turns an empty preview url into undefined, so no audio element is built', () => {
    expect(
      searchHitToBeatmap(hit({ previewUrl: '' }), difficulty()).previewUrl
    ).toBeUndefined();
  });

  it('leaves the round fields unset', () => {
    const beatmap = searchHitToBeatmap(hit(), difficulty());
