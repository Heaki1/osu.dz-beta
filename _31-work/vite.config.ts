...
export default defineConfig(({ mode }) => {
  ...
  '/uploads': {
    ...
    changeOrigin: true,
      ...
      watch: { ignored: ['**/.figma/**'] },
      // Let Vite derive the HMR protocol/port from the dev server itself. The previous
      // hard-coded wss://localhost:443 target failed when this project was opened directly
      // on its local HTTP port (8443), producing ERR_CONNECTION_REFUSED in the browser.
    },
    preview: {
      host: '0.0.0.0',
      port: parseInt(process.env.PORT || '8443'),
