...
export async function fetchPublicUserBestScores(userId: number | string, limit = 5): Promise<OsuBestScoreSummary[]> {
  ...
  const scores = (await res.json()) as Array<Record<string, any>>;
  return scores.map((score) => {
    const beatmap = (score.beatmap ?? {}) as Record<string, any>;
    return {
      id: Number(score.id),
      ...
      mods: Array.isArray(score.mods) ? score.mods.map((mod: any) => typeof mod === 'string' ? mod : mod?.acronym).filter(Boolean) : [],
      beatmap: {
        id: Number(beatmap.id),
        title: String(beatmap.full_name ?? beatmap.beatmapset?.title ?? (score.beatmapset as Record<string, any> | undefined)?.title ?? 'Unknown beatmap'),
