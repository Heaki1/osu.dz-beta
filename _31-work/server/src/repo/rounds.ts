...
const COLUMNS = `id, round_number, created_at, phase, month, year, reward,
...
export async function approveWinner(
  ...
  try {
    ...
    // Populate round_challenge_beatmaps: the top-N most-voted approved submissions
    // become the challenge beatmaps. N = site_settings.max_challenge_beatmaps; NULL = all.
    // The approved winner is always persisted as vote_rank = 1.
    const { settings } = await import('./siteSettings.js');
    const siteSettings = await settings();
    const { insertFromVotes } = await import('./challengeBeatmaps.js');
    await insertFromVotes(client, roundId, siteSettings.maxChallengeBeatmaps, winner);
