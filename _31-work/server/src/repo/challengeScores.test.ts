...
expect(
  qualifies(
    play('HD', 0, 80, 100),
    { modRequirement: 'HD', challengeRequirement: 'Full Combo' }
  )
).toBe(false);
...
expect(
  ...
  it('requires exactly the named mods — extra mods on the play cause a failure', () => {
    // HDHR does NOT satisfy HD: the player used an extra mod (HR).
    expect(qualifies(play('HDHR'), { modRequirement: 'HD', challengeRequirement: 'Top #1 Score' })).toBe(false);
    // HD alone satisfies HD.
    expect(qualifies(play('HD'),   { modRequirement: 'HD', challengeRequirement: 'Top #1 Score' })).toBe(true);
    // HD does NOT satisfy HDHR: missing HR.
    expect(qualifies(play('HD'),   { modRequirement: 'HDHR', challengeRequirement: 'Top #1 Score' })).toBe(false);
    // HRHD satisfies HDHR: same mods, different order.
    expect(qualifies(play('HRHD'), { modRequirement: 'HDHR', challengeRequirement: 'Top #1 Score' })).toBe(true);
    // HDDT does NOT satisfy HD.
    expect(qualifies(play('HDDT'), { modRequirement: 'HD', challengeRequirement: 'Top #1 Score' })).toBe(false);
  });
